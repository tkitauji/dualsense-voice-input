...
- The USB wrapper parses under Windows PowerShell 5.1, requires an actual USB PnP path before building, preserves a versioned timestamped report only for a started hardware run, and accepts only the probe's exact passing result. Its disconnected-controller guard returned the expected error without creating a report directory.
...
- Protocol self-test: Bluetooth button press/release, Bluetooth audio-report exclusion, USB button press/release, transport-layout separation, automatic-paste target safety, and interrupted-file cleanup all passed. Paste is accepted only when the original target still exists, differs from the app itself, and is the actual foreground window; mismatched, closed, and self targets are rejected. Startup cleanup removes only an app-prefixed WAV with an exact 32-digit GUID and the exact partial-model path while preserving an unrelated similarly named WAV and the valid model.
- Model startup recovery: an existing model file held with `FileShare.None` was treated as invalid without propagating the sharing-violation exception, so the app remains open and offers model recovery instead of crashing.
...
- Accessibility startup/UI Automation test: device selection, refresh, transcript editing, copy, and automatic-paste controls expose Japanese accessible names or help text; the published app stayed running and closed cleanly. Invoking refresh through UI Automation produced `LIVE_REGION_PASS|StatusText|DualSenseが見つかりません。接続して再読込してください`, proving that status changes raise the screen-reader `LiveRegionChanged` event with the changed message as its accessible name. Setting the transcript through UI Automation and invoking Copy produced `CLIPBOARD_COPY_PASS|dualsense-clipboard-probe` from the real Windows clipboard.
- The repeatable accessibility script runs under Windows PowerShell 5.1 and PowerShell Core 7.6, snapshots each existing clipboard format before its copy test, and restores the original data afterward. A sentinel-text pass confirmed exact restoration after the test process exited.
- Keyboard focus, hover, pressed, disabled, model connection/download/verification progress, and Windows high-contrast visual states are implemented. Normal-theme foreground/background contrast is at least 4.64:1 for interactive button text and 6.79:1 for secondary body text.
- WPF UI rendering self-test: the 760×570 main content produced 221,057 non-background pixels, the custom dark device selector displayed only `Wireless Controller — Bluetooth（直接）` rather than the backing object representation, its required popup template part remained present, and the empty transcript field no longer exposed a bright system scrollbar. The rebuilt published app then repeated the UI Automation live-region and real clipboard passes, and its rebuilt MSIX passed package preflight.
- Model download stream test: a deliberately awkward 4,096-byte payload copied with a 127-byte buffer was byte-identical at the destination, progress began at 0 and reached 100 without duplicate values, and an already-cancelled token stopped the copy with `OperationCanceledException`. Window close also propagates cancellation to Whisper.net's HTTP request and the file copy before partial-file cleanup.
- Model download recovery messages: network, timeout, access-denied, storage/I/O, and integrity failures are converted to actionable Japanese status text; protocol checks cover the network, storage, and integrity branches.
...
- Whisper optimized-runtime probe: forcing `Cpu` loaded `Cpu` and transcribed the synthesized Japanese test sentence exactly apart from punctuation.
- Whisper compatibility-runtime probe: forcing `CpuNoAvx` loaded `CpuNoAvx` and produced the same Japanese transcription. Runtime-order protocol checks select `Cpu` then `CpuNoAvx` on AVX/AVX2/FMA/F16C-capable CPUs and only `CpuNoAvx` otherwise.
- Native framework dependency: the MSIX manifest declares `Microsoft.VCLibs.140.00.UWPDesktop` version 14.0.30704.0 or newer for Store-managed installation.
- Store development preflight passed under PowerShell Core and Windows PowerShell 5.1: x64 identity/version shape, Windows 11 target, microphone and full-trust capabilities, packaged-desktop entry point, VC++ dependency, both optimized and No-AVX native payloads, required notices, and absence of incompatible runtimes were read from the actual MSIX. Submission mode correctly rejected the development identity and all eight missing owner/hardware/WACK/screenshot gates in one run. A temporary synthetic-evidence fixture exercised all three coexistence logs, USB log, all 20 manual checks, WACK `OVERALL_RESULT=PASS`, and a 1366×768 PNG; those gates passed while the real development identity remained rejected.
- Coexistence scenario guards now require no game clients for Baseline, the Steam client plus the named live game process (`MonsterHunterWilds` by default) for Steam, and `ffxiv_dx11` for FF14. All three disconnected/not-running guards failed before creating an evidence directory.
