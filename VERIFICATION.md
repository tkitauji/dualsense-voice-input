...
- Protocol self-test: Bluetooth button press/release, Bluetooth audio-report exclusion, USB button press/release, transport-layout separation, and automatic-paste target safety all passed. Paste is accepted only when the original target still exists, differs from the app itself, and is the actual foreground window; mismatched, closed, and self targets are rejected.
